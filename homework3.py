...
class AddressBook:
    def __init__(self):
        # add an empty dictionar for contacts
        self.contacts = {}

    def add_contact(self, name, phone):
        #check if name already exists, if not, add it
        if name in self.contacts:
            return f"Contact '{name}' already exists."
        else:
            # add contact and phone number and None for birthday
    ...
    def show_phone(self, name):
        # show phone number for a specified contact
        if name in self.contacts:
            return f"Phone number for contact '{name}': {self.contacts[name]['phone']}"
        else:
            return f"Contact '{name}' not found."
        
    def show_all(self): # show all contacts
